<?php
    ...
    class FPDF
    {
        ...
        function textIntoCols($strOriginal, $noCols, $pdf)
        {
            ...
            $iMaxCharRow = 400; //Número máximo de caracteres por renglón
        ...
        function TablaFactura($data)
        {
            // Colores, ancho de línea y fuente en negrita
            $this->SetFillColor(235, 245, 242);
            $this->SetTextColor(0);
            ...
            foreach ($data as $llave => $valor) {
                ...
                $this->Cell(85, 6, strtoupper($data[$llave][0]), 'LR', 0, 'C', $fill);
                $this->Cell(25, 6, $data[$llave][1], 'LR', 0, 'R', $fill);
                $this->Cell(30, 6, $data[$llave][2], 'LR', 0, 'R', $fill);
                $this->Cell(25, 6, $data[$llave][3], 'LR', 0, 'R', $fill);
//      $fill = !$fill;
                $this->Ln();
                $this->SetX(20);
                $this->Cell(165, 0, '', 'T');
        ...
        function TraePagos($Consecutivo, $Tipo, $Tipodoc, $SegConsecutivo = 0)
        {
            ...
            // Cabecera
            $this->SetX(20);
            $header = ['TIPO', 'ENTIDAD', 'NUMERO', 'VALOR'];
            $this->Cell(60, 7, $header[0], 1, 0, 'C', true);
            $this->Cell(45, 7, $header[1], 1, 0, 'C', true);
            $this->Cell(30, 7, $header[2], 1, 0, 'C', true);
            ...
            foreach ($Pagos as $llave => $valor) {
                $this->SetX(20);
                $this->SetFont('Arial', '', 9);
                $this->Cell(60, 6, utf8_decode($valor['NOMBRE_F_PAGO']), 'LR', 0, 'L', $fill);
                $this->Cell(45, 6, utf8_decode($valor['NOMBRE_ENTIDAD']), 'LR', 0, 'R', $fill);
                $this->Cell(30, 6, $valor['NUMERO'], 'LR', 0, 'R', $fill);
                $this->Cell(30, 6, number_format($valor['VALOR'], 0, '', ','), 'LR', 0, 'R', $fill);
                $Total += $valor['VALOR'];
//      $fill = !$fill;
                $this->Ln();
                $this->SetX(20);
            ...
            $this->SetFont('Arial', '', 9);
            $this->SetFont('', 'B');
            $this->Cell(135, 6, 'Total:', 'LR', 0, 'R', $fill);
            $this->Cell(30, 6, number_format($Total, 0, '', ','), 'LR', 0, 'R', $fill);
