<?php
    ...
    session_start();
    $Consecutivo = $_SESSION['ConsecutivoRECIBO'];
    ...
    $Elaboro = '';
    //print_r($_SESSION['ConsecutivoRECIBO']);
    $Anulado = '';
    foreach ($Documentos->TraeInformacionRecibo($_SESSION['ConsecutivoRECIBO'], $_SESSION['login'][0]["ID_EMPRESA"]) as $llave => $valor) {
    ...
    $pdf->Ln(4);
    $pdf->SetX(140);
    $pdf->Cell(40, 7, utf8_decode('RECIBO N°  ') . $Consecutivo, 1, 0, 'C', FALSE);
    $pdf->Ln(7);
    $pdf->SetX(147);
    ...
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(28, 4, 'Fecha Recibo: ', 'LR', 0, 'L');
    $pdf->SetFont('Arial', '', 9);
    ...
    $pdf->Cell(115, 6, "Pago de Factura No." . $_SESSION['Pagos'], 0, 1, 'L', true);
    $pdf->SetX(20);
    $pdf->SetFont('Arial', '', 9);
